...
if __name__ == '__main__':
    ...
    rbf_norm_scores = np.load('../data/KernelSVMNorm-PCA11-RegBias0.05-Gamma0.01-Bound1Scores.npy')
    rbf_raw_scores = np.load('../data/KernelSVM-PCA10-RegBias0.05-Gamma0.001-Bound1Scores.npy')
    ...
    dcf_before, _ = utils.DCF(rbf_norm_scores, labels)
    mindcf, _ = utils.minDCF(rbf_norm_scores, labels)
    dcf_after, _ = utils.DCF(cal > 0, labels)
    print(f"RBF Norm Poly minDCF {mindcf} DCF Before {dcf_before} DCF After {dcf_after}")
